...
def viewCompany(dbObj) :
    ...
    if option == 'Name':
        ...
        if search :
            companyDetails = dbObj.fetchCompanyReviewByName(companyName)
            if len(companyDetails) == 0:
                ...
                for x in companyDetails:
                    ...
                    stars = st_star_rating(label="Rating", maxValue=5, defaultValue=rating, key="rating",
        ...
        search = st.button('Search')
        if search :
            companyDetails = dbObj.fetchCompanyReviewByLocation(companyLocation)
            if len(companyDetails) == 0 :
                st.write('No Results Found')
            else :
                st.write('<h2>Results<h2>', unsafe_allow_html=True)
...
def viewPlace(dbObj) :
    ...
    if option == 'Name':
        ...
        search = st.button('Search')
        if search :
            placeDetails = dbObj.fetchPlaceReviewByName(placeName)
        ...
        search = st.button('Search')
        if search :
            placeDetails = dbObj.fetchPlaceReviewByLocation(placeLocation)
            if len(placeDetails) == 0 :
                st.write('No Results Found')
            else :
                st.write('<h2>Results<h2>', unsafe_allow_html=True)
